...
parser.add_argument("--use_dorsal", action='store_true',
                    help='If true, use dorsal ROI from NSD for fine-tuned models. Else use ventral.')
parser.add_argument("--save_encoding_weights", action='store_true')
...
def main():
    ...
    save_encoding_weights = args.save_encoding_weights
    ...
    for roi in rois:
        ...
        if method == "untuned":
            ...
            reg_model_dir = os.path.join(
                project_dir, "baseline_models", "nn_reg", "Subj1")
        ...
        if save_encoding_weights:

            if method != "cl-tuned":
                print("Saving encoding weights only supported for CL-tuned models.")
                return

            encoding_weights_save_dir = os.path.join(
                project_dir, "nod", "encoding_weights",  "Subj" + str(subj_num)
            )
            encoding_weights_save_file = os.path.join(
                encoding_weights_save_dir,  "encoding_weights.joblib")
            ...
            # Load best layer for ROI
            best_alex_layers_and_alphas_dict = joblib.load(
                existing_results_dict_path)
            layer, _ = best_alex_layers_and_alphas_dict[hemisphere_abbr + "h_" + roi]
            ...
            # Load existing weights dictionary or create new one
            try:
                encoding_weights_dict = joblib.load(encoding_weights_save_file)
            except:
                encoding_weights_dict = {}

            # Check if this ROI is empty
            if num_voxels == 0:
                print("ROI empty")
                encoding_weights_dict[hemisphere_abbr + "h_" + roi] = -1

            else:
                # Load alpha for each voxel in ROI
                best_alphas_voxel_dict = joblib.load(
                    best_alphas_voxel_dict_path)
                layer_voxel_alphas = best_alphas_voxel_dict[hemisphere_abbr + "h_" + roi]

                # Fit encoding model
                encoding_model = Ridge(alpha=layer_voxel_alphas, random_state=0).fit(
                    train_features, train_fmri)

                # Save encoding model weights
                encoding_weights_dict[hemisphere_abbr +
                                      "h_" + roi] = encoding_model.coef_

            joblib.dump(encoding_weights_dict, encoding_weights_save_file)
            ...
            for fold_num in range(5):
                ...
                for layer_idx, layer in enumerate(alex_out_layers):
                    ...
                    print(roi + " layer:" + layer +
                          " acc: " + str(layer_avg_acc))
            ...
            if num_voxels != 0:
                ...
                joblib.dump(best_alphas_voxel_accs_dict,
                            best_alphas_voxel_accs_dict_path)
                joblib.dump(best_alphas_voxel_dict,
                            best_alphas_voxel_dict_path)
