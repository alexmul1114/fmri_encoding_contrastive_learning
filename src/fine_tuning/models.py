...
class pooled_CLR_model(torch.nn.Module):
    ...
    def __init__(self, num_voxels_each_subj, h_dim, z_dim):
        ...
        self.alex_h = nn.Linear(1000, h_dim, bias=False)
        self.fmri_hs = nn.ModuleList([nn.Linear(num_voxels, h_dim, bias=False) for num_voxels in num_voxels_each_subj])
...
def train_pooled(model, device, train_dataloaders_subjs, optimizer, epochs, temp, val_dataloader=None):
    ...
    # Training loop
    for epoch in tqdm(range(epochs)):
        total_train_loss = train_one_epoch_pooled(model, device, train_dataloaders_subjs, optimizer, temp)
        print("Epoch " + str(epoch) + " train loss: " + str(total_train_loss.item()))
        if (val_dataloader is not None):
            total_val_loss = val_loss(model, val_dataloader, temp, device)
            print("Epoch " + str(epoch) + " val loss: " + str(total_val_loss.item()))
