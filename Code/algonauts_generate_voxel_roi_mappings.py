...
# Local imports
from algonauts_utils import get_voxel_roi_mappings, get_dataloaders
...
if __name__ == '__main__':
    ...
    for subj_num in tqdm(range(1,9)):
        ...
        save_folder = project_dir + "/voxel_roi_mappings/Subj" + str(subj_num) 
        ...
        voxel_roi_lists = get_voxel_roi_mappings(project_dir, subj_num, 'left', num_voxels)
        ...
        train_dataloader, train_size, num_voxels = get_dataloaders(project_dir, device, subj_num, 'right', 'all', 
                                                                    batch_size=1024, voxel_subset=False, voxel_subset_num=0, use_all_data=True)
        # Get list of voxel-ROI mappings
        voxel_roi_lists = get_voxel_roi_mappings(project_dir, subj_num, 'right', num_voxels)
