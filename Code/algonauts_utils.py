...
class algoDataSet(Dataset):
    ...
    def __init__(self, data_dir, device, subj_num, hemisphere, roi_name, voxel_subset=False, voxel_subset_num=0):
        ...
        # Path to COCO ids mapping file, load
        nsd_stim_info_path = data_dir + "/nsd_stim_info_merged.pkl"
        stiminfo = pd.read_pickle(nsd_stim_info_path)
        
        # Save corresponding coco image ids in array (same order as image paths)
        coco_ids = np.zeros(len(self.img_paths), dtype=np.int32)
        counter = 0
        for path in self.img_paths:
            nsd_id = int(str(path)[-9:-4])
            coco_id = int(stiminfo.iloc[nsd_id]['cocoId'])
            coco_ids[counter] = coco_id
            counter += 1
        # Save coco ids
        coco_save_file_name = data_dir + "/coco_data/subj" + str(subj_num) + "_coco_ids.npy"
        np.save(coco_save_file_name, coco_ids)
        
        # Load coco ids
        coco_save_file_name = data_dir + "/coco_data/subj" + str(subj_num) + "_coco_ids.npy"
        coco_ids = np.load(coco_save_file_name)
        coco_people_array = np.load("/home/alexmulrooney/algonauts_data/coco_data/coco_people_array.npy")
        people_labels = np.zeros(len(coco_ids))
        # Generate people/not people labels for binary classification
        for idx in range(len(coco_ids)):
            coco_id = coco_ids[idx]
            contains_people = coco_people_array[coco_id]
            if (contains_people==1):
                people_labels[idx] = 1
        people_labels_save_path = "/home/alexmulrooney/algonauts_data/coco_data/subj" + str(subj_num) + "_people_labels.npy"
        np.save(people_labels_save_path, people_labels)
